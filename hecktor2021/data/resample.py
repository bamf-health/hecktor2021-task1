...
def resample_one_patient(p, bb_df, resampling, input_folder, output_folder):
    ...
    ct = sitk.ReadImage(str([f for f in input_folder.rglob(p + "_ct*")][0].resolve()))
    pt = sitk.ReadImage(str([f for f in input_folder.rglob(p + "_pt*")][0].resolve()))
...
def main(input_folder, output_folder, bounding_boxes_file, cores, resampling):
    ...
    patient_list = [
        f.name.split(".")[0].split("_")[0] for f in input_folder.rglob("*.nii.gz")
    ]
    ...
    if cores == 1:
        ...
        args = [
            (p, bb_df, resampling, input_folder, output_folder) for p in patient_list
        ]
        with Pool(cores) as p:
            p.starmap(resample_one_patient, args)
